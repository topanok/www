	                                ...
	                                <ul>
	                                    ...
	                                    <?php 
	                                        ...
	                                        for ($i=0; $i<count($data['categories']); $i++){
	                                        	if ($data['categories'][$i]->getParent_id() == 0) {
	                                        		 $str.='<li class="arrow-plus">';
	                                       			$str.= '<a href="http://localhost/products/see/'.$data['categories'][$i]->getId().'/1">'.$data['categories'][$i]->getName().'</a>';
	                                       			$str.='<div class="cat-left-drop-menu">';
														for ($j=0; $j<count($data['categories']); $j++){
	                                       					if($data['categories'][$j]->getParent_id()==$data['categories'][$i]->getId()){
	                                       						$str.='<div class="cat-left-drop-menu-left">';
	                                       						$str.='<a class="menu-item-heading" href="http://localhost/products/see/'.$data['categories'][$j]->getId().'/1">'.$data['categories'][$j]->getName().'</a>';
	                                       						$str.='<ul>';
	                                       						for($k=0; $k<count($data['categories']); $k++){
		                                       						if($data['categories'][$k]->getParent_id()==$data['categories'][$j]->getId()){
		                                       							$str.='<li><a href="http://localhost/products/see/'.$data['categories'][$k]->getId().'/1">'.$data['categories'][$k]->getName().'</a></li>';
		                                       						}
		                                       					}
	                                       						$str.='</ul>';
	                                       						$str.='</div>';
